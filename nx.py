...
# load_dir = "datasets_with_secbl"  # mean second shell distance
load_path = [
    os.path.join(load_dir, file_names[i]) for i in range(len(file_names))
]
tcombined_data.pklargets = ["cs", "cn", "bl"]
# targets = ["bl_2nd"]
...
def main(trained_database, random_seed=40, model_type="rf"):
    ...
    for j in range(len(total_outs)):
        keys = [total_outs[j][key] for key in keynames]
        value = {key: total_outs[j][key] for key in result_valuekeys}
...
if __name__ == "__main__":
    ...
    with open("results/combined_data.pkl", "rb") as f:
        previous_combined_data = pickle.load(f)

    previous_results_data = previous_combined_data["results"]

    out = main(previous_results_data, 40, "rf")
