...
def filter_data(data, conditions):
    ...
    for i in range(len(conditions)):
        mask = np.array(list(map(lambda x: conditions[i](x), data)), dtype='bool')
        # print(mask)
        masks *= mask
...
class SmartPloter():
    ...
    def bar(self, ax=None, **plot_params):
        ...
        if self.counter == 1:
            ax.bar(x=x0, height=metrics, width=2/3, yerr=errors, **plot_params)
        else:
            width = 1/(self.counter+1)
            for i in range(self.counter):
                x = x0+i*width
                h = ax.bar(x=x, height=metrics[i], width=width, yerr=errors[i], **plot_params)
                handles.append(h)
                for j in range(len(x)):
                    ax.text(x[j], metrics[i][j], "{:.3f}".format(metrics[i][j]), horizontalalignment='center')

        return ax, handles


def bunch_bar_plot(data, key, first_level_label, second_level_labels, third_level_labels, fig=None, axes=None):
    """
    ...
    for j, scd in enumerate(second_level_labels):
        ...
        for k, thd in enumerate(third_level_labels):
            scores = lookup(data, key, target=first_level_label, element=scd, feature=thd)
            means.append(np.mean(scores))
            stds.append(np.std(scores))
        bar = axes[j].bar(range(len(third_level_labels)), means, yerr=stds)
        bars.extend(bar)
        for h in range(len(means)):
            axes[j].text(range(len(third_level_labels))[h], means[h], "{:.3f}".format(means[h]))

    return fig, axes, bars

def feature_importances_plot(data, key, first_level_label, second_level_labels, third_level_labels, fig=None, axes=None):
    """
    ...
    for k, thd in enumerate(third_level_labels):
        values = lookup(data, key, target=first_level_label, element=second_level_labels, feature=thd)
        means = np.mean(values, axis=0)
        axes[k].plot(range(len(values[0])), *values, color='gray', alpha=0.2)

        for h in range(int(len(means)//100)):
            axes[k].plot(np.arange(h*100, h*100+100), means[h*100:h*100+100], color=colors[1+h])

        peaks,_ = find_peaks(means, height=0.02)
        ylim = axes[k].get_ylim()
        axes[k].set_xlim([0,300])
        for ind in peaks:
            axes[k].vlines(np.arange(len(values[0]))[ind],*ylim, color='k', linestyle='--', linewidth=0.5)
...
def bunch_hline_plot(data, key, first_level_label, second_level_labels, third_level_labels, fill=False, fig=None, axes=None):
    """
    ...
    for j, scd in enumerate(second_level_labels):
        ...
        for k, thd in enumerate(third_level_labels):
            ...
            std  = np.std(scores)
            if fill:
                axes[j].fill_between(np.linspace(*xlim, 100), (mean-std/2)*np.one(100), (mean+std/2)*np.ones(100))
            else:
...
def set_ax_style(ax, xticks=None, yticks=None,xlim=None, ylim=None, xlabel=None, ylabel=None, title=None):
    ...
    if xticks is not None:
        ...
        plt.setp(ax.get_xticklabels(), rotation=45, fontsize=10, ha='center')
