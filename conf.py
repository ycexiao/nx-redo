...
data_path, random_seeds = sort_files(
    find_files("active-data-house", is_data),
    random_seed_pattern,
    return_keys=True,
)

is_track = lambda x: x.count("-") == 7 and x.startswith("data-split-track")
track_path = sort_files(
    find_files("active-data-house", is_track), random_seed_pattern
)

is_results = lambda x: x.count("-") == 5 and x.startswith("results")
results_path = sort_files(
    find_files("active-data-house", is_results), random_seed_pattern
)
